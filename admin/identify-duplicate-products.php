<?php
/**
 * Identify duplicate products by reference number
 * and flag items missing key information (HCPCS, size)
 */
...
if (!empty($to_delete)) {
  ...
  echo "Individual DELETE commands:\n";
  foreach ($to_delete as $id) {
    $product = array_filter($products, fn($p) => $p['id'] == $id)[0] ?? null;
    if ($product) {
      echo "-- DELETE ID $id: {$product['name']}\n";
      echo "UPDATE products SET active = FALSE WHERE id = $id;\n\n";
    }
  }
}
