  ...
  <?php if ($isOfficeStock): ?>
    ...
    <div style="border: 1px solid var(--border); border-radius: var(--radius); padding: 1.5rem; margin-bottom: 2rem; background: white;">
      ...
      <div id="office-stock-products">
        ...
        ?>
          <div class="product-row office-stock-row" style="display: grid; grid-template-columns: 1.5fr 1fr 0.6fr 0.6fr 0.8fr 0.8fr auto; gap: 0.5rem; align-items: end; padding: 0.75rem; background: var(--bg-gray); border-radius: var(--radius); margin-bottom: 0.5rem;">
            <!-- Hidden field to store selected product ID -->
            <input type="hidden" name="products[0][<?= $prodIndex ?>][product_id]" class="product-id-input" value="<?= htmlspecialchars($prod['product_id'] ?? '') ?>">
            ...
            <div>
              ...
              <select class="size-select" onchange="updateProductIdOfficeStock(this)"
            ...
            <div>
              ...
              <input type="number" name="products[0][<?= $prodIndex ?>][boxes]" value="<?= htmlspecialchars($prod['boxes'] ?? '1') ?>" min="1" class="boxes-input" onchange="updateOfficeStockRowPrice(this)"
                     style="width: 100%; padding: 0.5rem; font-size: 0.875rem; border: 1px solid var(--border); border-radius: var(--radius);">
            </div>
            <div>
            ...
            <div>
              <button type="button" onclick="removeOfficeStockRow(this)"
...
function addOfficeStockRow() {
  ...
  const row = document.createElement('div');
  row.className = 'product-row office-stock-row';
  row.style.cssText = 'display: grid; grid-template-columns: 1.5fr 1fr 0.6fr 0.6fr 0.8fr 0.8fr auto; gap: 0.5rem; align-items: end; padding: 0.75rem; background: var(--bg-gray); border-radius: var(--radius); margin-bottom: 0.5rem;';
  ...
  row.innerHTML = `
    <input type="hidden" name="products[0][${prodIndex}][product_id]" class="product-id-input" value="">
    <div>
      <label style="display: block; font-size: 0.75rem; font-weight: 500; margin-bottom: 0.25rem;">Product Type</label>
      <select class="product-type-select" onchange="updateSizeDropdown(this)"
              style="width: 100%; padding: 0.5rem; font-size: 0.875rem; border: 1px solid var(--border); border-radius: var(--radius);">
    ...
    <div>
      <label style="display: block; font-size: 0.75rem; font-weight: 500; margin-bottom: 0.25rem;">Boxes</label>
      <input type="number" name="products[0][${prodIndex}][boxes]" value="1" min="1" class="boxes-input" onchange="updateOfficeStockRowPrice(this)"
             style="width: 100%; padding: 0.5rem; font-size: 0.875rem; border: 1px solid var(--border); border-radius: var(--radius);">
    </div>
    <div>
      <label style="display: block; font-size: 0.75rem; font-weight: 500; margin-bottom: 0.25rem;">Pcs/Box</label>
...
function updateProductIdOfficeStock(sizeSelect) {
  const row = sizeSelect.closest('.product-row');
  const typeSelect = row.querySelector('.product-type-select');
  const productIdInput = row.querySelector('.product-id-input');
  const productType = typeSelect.value;
  ...
  if (productType && size && productsByType[productType] && productsByType[productType][size]) {
    const product = productsByType[productType][size];
    productIdInput.value = product.id;
    updateOfficeStockRowPrice(row);
  } else {
    productIdInput.value = '';
    row.querySelector('.pieces-per-box').textContent = '-';
    row.querySelector('.price-per-box').textContent = '-';
    row.querySelector('.row-total').textContent = '-';
    updateOfficeStockTotal();
  }
}

function updateOfficeStockRowPrice(element) {
  const row = element.closest ? element.closest('.product-row') : element;
  if (!row) return;
...
function updateOfficeStockTotal() {
  const rows = document.querySelectorAll('#office-stock-products .product-row');
  let total = 0;
  ...
  // Display total somewhere if needed (could add a total section for office stock)
  console.log('Office Stock Total:', total);
}

function removeOfficeStockRow(button) {
  const row = button.closest('.product-row');
  row.remove();
  updateOfficeStockTotal();
}

function initializeOfficeStockRow(row) {
  const productIdInput = row.querySelector('.product-id-input');
  const typeSelect = row.querySelector('.product-type-select');
  const sizeSelect = row.querySelector('.size-select');
...
document.addEventListener('DOMContentLoaded', function() {
  <?php if ($isOfficeStock): ?>
    // Initialize existing office stock rows or add initial row
    const existingOfficeRows = document.querySelectorAll('#office-stock-products .product-row');
    if (existingOfficeRows.length === 0) {
      addOfficeStockRow();
    } else {
      // Initialize existing rows with cascading dropdowns
      existingOfficeRows.forEach(row => initializeOfficeStockRow(row));
      updateOfficeStockTotal();
    }
  <?php else: ?>
