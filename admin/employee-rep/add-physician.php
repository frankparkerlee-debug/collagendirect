<?php
/**
 * Employee Sales Rep: Add Physician to Clinic
 *
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$error) {
    try {
      ...
      if ($emailSent) {
        error_log("[employee-rep/add-physician] Welcome email sent to $email");
        ...
        error_log("[employee-rep/add-physician] Failed to send welcome email to $email");
      ...
      $pdo->rollBack();
      error_log("Employee rep add physician error: " . $e->getMessage());
...
<?php if ($message): ?>
  <div class="card p-4 mb-6 bg-green-50 border-green-200">
    ...
    <div class="mt-3">
      ...
      <a href="/admin/employee-rep/add-physician.php" class="btn ml-2">Add Another</a>
    </div>
  </div>
<?php endif; ?>
...
<?php if (empty($practices)): ?>
  <div class="card p-8 text-center">
    ...
    <a href="/admin/employee-rep/onboard-clinic.php" class="btn btn-primary">Onboard a Practice First</a>
  </div>
<?php elseif (!$message): ?>
